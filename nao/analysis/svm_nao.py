...
for i in range(0,np.shape(y_train_raw)[0]):
  ...
  #a = [1E-15]
  C = np.logspace(np.log(0.1)/np.log(10),np.log(100)/np.log(10),endpoint=True,num=5,base=10.0)
  #alpha_op = np.hstack((a,b))

  #'''
  ## Grid search
  clf = svm.SVR(C=0.0,kernel='linear')
  param_dist = {'C': C}
  ...
  ## Running
  reg = reg = svm.SVR(C=best_params.items()[0][1], kernel='linear')
  ...
  e = open('svma_regression_scaife3.csv', 'a+')
  e.write(str(p_test[0]) + '\n')
  e.close()

  del(p_test, clf, reg, x_train_norm, x_train, y_train, a, b, x_test, mypullout, scaler, C, param_dist, grid_search, best_params)
